...
public class Impl4Abstract extends ImplementorForAbstractClasses {
	 ...
	 * Templates.
	 */
	private ChildPropertyWithDirectAccessCodeTemplates directChildProperty = new ChildPropertyWithDirectAccessCodeTemplates();

	/**
	 * Templates.
	 */
	private ChildPropertyWithReflectiveAccessCodeTemplates reflectiveChildProperty = new ChildPropertyWithReflectiveAccessCodeTemplates();

	/**
	 * Templates.
	 */
	private PropertyWithDelegateKeyCodeTemplates delegateKeyProperty = new PropertyWithDelegateKeyCodeTemplates();

	/**
	 * Templates.
	 */
	private FieldInitializationCodeTemplates initField = new FieldInitializationCodeTemplates();
	...
	/**
	 * Analyzes {@link DelegateKeyProperties} annotation of the fields of the
	 * class being processed.
	 *
	 * @throws ClassCreationException
	 *             the class creation exception
	 */
	void analyzeDelegateKeyProperties() throws ClassCreationException {
		Class<?> clazz = analysis.getClazz();
		DelegateKeyProperties anno = clazz.getAnnotation(DelegateKeyProperties.class);
		if (anno != null) {
	...
	/**
	 * Analyzes {@link TypedSelectableProperties} annotation of the fields of
	...
	void analyzeTypeSelectableProperties() throws ClassCreationException {
		...
		if (TypedSelectable.class.isAssignableFrom(clazz)) {
			TypedSelectableProperties anno = clazz.getAnnotation(TypedSelectableProperties.class);
			if (anno != null) {
	...
	/**
	 * Method that distinguishes the annotation properties based on the given
	 * value. If the annotation's property value is empty or equal to "X" then
	 * the corresponding property will be ignored. Otherwise, the property
	 * accessors will be included and will be delegated to the specified
	 * class property.
	 *
	 * @param property
	 *            the property
	...
	void markTsProperty(String property, String replacement) throws ClassCreationException {
		boolean isEmpty = StringUtils.isNullOrBlank(replacement) || "X".equalsIgnoreCase(replacement); //$NON-NLS-1$
		if (!isEmpty) {
	...
	ClassCreationException invalidDelegateKeyProperties() {
		@SuppressWarnings("nls")
		String msg = StringUtils.concat("Class ", analysis.getClazz().getName(),
	...
	ClassCreationException invalidTypedSelectableProperties(String invalidName) {
		@SuppressWarnings("nls")
		String msg = StringUtils.concat("Class ", analysis.getClazz().getName(), " references the invalid property ",
				invalidName, " in the TypeSelectableProperties annotation");
		return new ClassCreationException(msg);
	}
	...
	void supportPo() {
		...
		Map<String, String> initKeyVars = Variables.variablesForInitialization(keyType, "key", createKey);
		...
		if (children != null) {
			for (Field field : children) {
				if (Set.class.isAssignableFrom(field.getType())) {
					Map<String, String> vars = Variables.variablesForInitialization(field.getType(), field.getName(),
							value);
					String initCode;
					String fixCode;
		...
		AddingCodeToDefaultConstructorClassUpdater updater = new AddingCodeToDefaultConstructorClassUpdater(
				constructorCode);
		addUpdater(updater);
	}
	...
	protected boolean supportComparable() throws ClassCreationException {
		...
		if (!ok) {
			if (TypedSelectable.class.isAssignableFrom(analysis.getClazz())) {
				BeanPropertyDefinition<?>[] defs = { analysis.getFirstPropertyByName("typeId"),
			...
			if (Codified.class.isAssignableFrom(analysis.getClazz())) {
				BeanPropertyDefinition<?>[] defs = { analysis.getFirstPropertyByName("code") };
