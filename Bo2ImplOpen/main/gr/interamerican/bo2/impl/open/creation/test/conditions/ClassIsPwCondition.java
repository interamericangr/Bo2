...
/**
 * Condition to check if a class is a {@link PersistentObject}.<br>
 * If {@link Class#forName(String)} throws an {@link ClassNotFoundException} -
 * then it will be wrapped to a {@link RuntimeException}.
 */
public class ClassIsPwCondition implements Condition<String> {

	@Override
	public boolean check(String clzName) {
		try {
			Class<?> clz = Class.forName(clzName);
			return PersistentObject.class.isAssignableFrom(clz);
