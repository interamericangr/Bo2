...
implements Cache<C>, Serializable {
	...
	/**
	 * Creates a unique key for the pair of typeId and subTypeId of a
	 * {@link TypedSelectable} value.
	 *
	 *
	 * @param value
	 *            the value
	...
	public void remove(TypedSelectable<C> value) {
		...
		if (cache.remove(k) != null) {
			...
			Pair<Long, Long> allEntriesKey = subKey(value.getTypeId(), SUBTYPEID_FOR_ALL_TYPE_ENTRIES);
	 ...
	 */
	<T extends TypedSelectable<C>, K> Set<T> getSubset(K key, Map<K, Set<TypedSelectable<C>>> map) {
