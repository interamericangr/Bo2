...
extends BPanel<MultiThreadedLongProcess> {
	...
	/** The batch process stats panel. */
	BatchProcessStatsPanel batchProcessStatsPanel;

	/**
	 *
	 */
	BatchProcessExecutionStatusPanel batchProcessExecutionStatusPanel;
	/**
	...
	void defineLayout(Component reference) {
		 ...
		 */
		layout.putConstraint(NORTH, sysInfo, 5, NORTH, this);
		layout.putConstraint(NORTH, statsPanel, 5, NORTH, this);
		layout.putConstraint(NORTH, batchProcessStatsPanel, 5, NORTH, this);
		layout.putConstraint(NORTH, newThread, 50, NORTH, this);
		...
		layout.putConstraint(WEST, batchProcessStatsPanel, 20, EAST, sysInfo);
		layout.putConstraint(WEST, newThread, 20, EAST, batchProcessStatsPanel);
		layout.putConstraint(WEST, refresh, 20, EAST, batchProcessStatsPanel);
		/*
		 * buttons below batchProcessStatsPanel
		 */
		layout.putConstraint(NORTH, refresh, 20, SOUTH, newThread);
		//
		layout.putConstraint(NORTH, batchProcessExecutionStatusPanel, 5, SOUTH, batchProcessStatsPanel);
		layout.putConstraint(WEST, batchProcessExecutionStatusPanel, 10, EAST, sysInfo);
		/*
		 * Status labels and the rest one under the reference panel
		 * (reference is the highest of sysInfo and statsPanel).
		 */
		layout.putConstraint(NORTH, statusLabelsPanel, 20, SOUTH, reference);
		layout.putConstraint(NORTH, aggregatePanel, 5, SOUTH, statusLabelsPanel);
	}
	...
	void drawBasicComponents() {
		...
		batchProcessStatsPanel = new BatchProcessStatsPanel(model);
		add(batchProcessStatsPanel);
		batchProcessExecutionStatusPanel = new BatchProcessExecutionStatusPanel(new BatchProcessExecutionStatusPanelModel(model));
		add(batchProcessExecutionStatusPanel);
	...
	public void model2panel() {
		...
		batchProcessExecutionStatusPanel.model2panel();
		batchProcessStatsPanel.model2panel();
