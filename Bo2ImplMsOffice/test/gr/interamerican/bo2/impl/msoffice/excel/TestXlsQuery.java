...
public class TestXlsQuery {
	 ...
	 */
	@Test
	public void testFormulas() throws DataException, LogicException, UnexpectedException {
		new AbstractBo2RuntimeCmd() {

			@Override
			public void work() throws LogicException, DataException, InitializationException, UnexpectedException {
				String manager = "LOCALFS"; //$NON-NLS-1$
				String streamName = "formula.xls"; //$NON-NLS-1$
				XlsQuery q = new XlsQuery(streamName, 0);
				q.setManagerName(manager);
				q.init(getProvider());
				q.open();
				...
				q.next();
				XlsRow row = q.getXlsRow();
				int one = row.getInt(0);
				int two = row.getInt(1);
				int three = row.getInt(2);
