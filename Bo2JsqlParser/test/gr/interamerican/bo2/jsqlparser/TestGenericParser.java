...
public class TestGenericParser extends AbstractParserTest {
	...
	public void testRemoveParameter_equalsSubSelect2() throws SqlParseException {
		String sql = StringUtils.concat( 
				...
				"on A.doy = B.doy ",
				"where A.surname like :surname ",
				"and A.salary = (select max(C.salary) from xxxx.SALARIES as C where C.doyNm = :doyNm and C.foo = 1) ",
				"and B.doyNm like :doyNm");
		...
		String expected = StringUtils.concat( 
				...
				"inner join xxxx.DOY as B ",
				"on A.doy = B.doy ",
				"where A.salary = (select max(C.salary) from xxxx.SALARIES as C where C.doyNm = :doyNm and C.foo = 1) ",
				"and B.doyNm like :doyNm");
		checkStatement(expected, actual);
